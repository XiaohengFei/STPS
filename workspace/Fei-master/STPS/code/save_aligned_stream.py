...
try:
    while True:
        ...
        cv2.imwrite(f"data/color_{idx}.png", color_image)
        cv2.imwrite(f"data/depth_{idx}.png", depth_image)
        idx += 1
        time.sleep(0.2)
        
        # Save NumPy arrays
        np.save(f"data/color_{idx}.npy", color_image)
        np.save(f"data/depth_{idx}.npy", depth_image)
